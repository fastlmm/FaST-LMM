...
class TestTwoKernelFeatureSelection(unittest.TestCase):
    ...
    def cmktest_regression_lmm(self):
    ...
    def cmktest_regression_lr(self):
        ...
        for outfile in os.listdir(output_dir):
            ...
            if (
                ...
                if not out:  # cmk
                    ...
                    msg = f"tempOutfile={Path(tmpOutfile).read_text()} referenceOutfile={Path(referenceOutfile).read_text()}"
                    # raise Exception(msg)
                # if not out:
