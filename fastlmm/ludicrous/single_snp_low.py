...
from fastlmm.ludicrous.bigsvd import big_sdd
...
import random
from fastlmm.ludicrous.mmultfile import mmultfile_b_less_aatb,get_num_threads,mmultfile_ata
from fastlmm.ludicrous.to_pysnptools import SnpMemMap
from pysnptools.util.intrangeset import IntRangeSet
from fastlmm.ludicrous.file_cache import log_in_place
from fastlmm.ludicrous.distributed_bed import DistributedBed
from fastlmm.ludicrous.file_cache import _format_delta
from fastlmm.ludicrous.file_cache import LocalCache
from fastlmm.ludicrous.file_cache import progress_reporter
from fastlmm.util.mapreduce import _identity, map_reduce, _dyn_vars
...
def postsvd(chrom_list, gtg_npz_lambda, memory_factor, cache_dict, G0_iid, G0_sid, G0_memmap_lambda, ss_per_snp, RxY, X, postsvd_runner, clear_local_lambda, min_work_count, log_frequency=-1):
    ...
    def mapper_closure(chrom):
        ...
        assert not chrom_storage.file_exists(fn_U), "if already done, don't call this"
...
def __del__(test_snps):
    ...
    from fastlmm.ludicrous.to_pysnptools import _MergeRows, _MergeCols
    from fastlmm.ludicrous.distributed_bed import _Distributed1Bed, DistributedBed
...
def node_sequence():
    skip_set = {"GCRCM02B04","GCRCM02B07","GCRCM02B13","GCRCM02B14","GCRCM02B15","GCRCM02B16","GCRCM03B04","GCRCM03B05","GCRCM03B13","GCRCM03B14","GCRCM03B15","GCRCM03B16","GCRCM04B13","GCRCM04B14","GCRCM04B15",
                "GCRCM05B04","GCRCM05B13","GCRCM05B14","GCRCM05B15",
                "GCRCM06B04","GCRCM06B13","GCRCM06B14","GCRCM06B15","GCRCM06B16","GCRCM07B13","GCRCM07B14","GCRCM07B15",
                "GCRCM08B13","GCRCM08B14","GCRCM08B15","GCRCM09B04","GCRCM09B13","GCRCM09B14","GCRCM09B15","GCRCM09B16","GCRCM10B13","GCRCM10B14","GCRCM10B15","GCRCM11B13","GCRCM11B14","GCRCM11B15","GCRCM12B04","GCRCM12B13","GCRCM12B14","GCRCM12B15","GCRCM12B16","GCRCM13B11","GCRCM13B13","GCRCM13B14",
                "GCRCM13B15","GCRCM13B22","GCRCM14B04","GCRCM14B08","GCRCM14B09","GCRCM14B13","GCRCM14B14","GCRCM14B15","GCRCM15B04","GCRCM15B13","GCRCM15B14","GCRCM15B15","GCRCM15B16"}
    for first in xrange(2,16):
        for second in xrange(4,25):
            s = "GCRCM{0:02d}B{1:02d}".format(first,second)
            if s not in skip_set:
                yield s

pattern2 = r"\\{0}\Scratch\escience\carlk\data\carlk\cachebio\genetics{1}"
...
def single_snp2(cache_dict,test_snps,pheno,G0,covar,memory_factor=1,
...
#!!!1 this is useful, move to fastlmm's mapreduce.py (with better name)
